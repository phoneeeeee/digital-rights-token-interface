...
func (tc *TokenContract) InvokeContract(stub shim.CMStubInterface) protogo.Response {
	...
	default:
		return shim.Error("[TokenContract] invalid method")
	}
}
...
func (tc *TokenContract) BuildTokenIssueTx(stub shim.CMStubInterface) protogo.Response {
	...
	rolesStr := string(args["roles"])                  // JSON数组
...
func (f *FactContract) BuildPublishTokenTx(stub shim.CMStubInterface) protogo.Response {
	// 1. 获取参数
	args := stub.GetArgs()
	...
	// 9. 返回成功
	return shim.Success([]byte("[buildPublishTokenTx] success for token: " + tokenName))
}
...
// BuildModifyAuthenticationInfoTx
// (2) 修改通证确权信息
// 文档: buildModifyAuthenticationInfoTx({account, tokenId, authenticationInfo})
...
func (tc *TokenContract) BuildModifyConstraintTx(stub shim.CMStubInterface) protogo.Response {
	...
	// 2. 多签检查: 版权单元记录的所有地址, 是否都在 constraintUpdate.Signers 里?
...
func (tc *TokenContract) BuildTokenChangeTx(stub shim.CMStubInterface) protogo.Response {
	...
	// 2. 校验权限: “account”是否真有修改该token的权限？
	//    业务可自定义. 这里示例直接允许
